        ...
        return [
            {
                ...
                getItems() {
                    return getAlgoliaResults({
                        searchClient,
                        queries: [
                            {indexName: ALGOLIA_FAQ_INDEX_NAME, query, params: {hitsPerPage: 1}}
                        ]
                    })
                },
 ...
 */
const ItemBreadcrumb = memo<{hit: FaqHit; components: AutocompleteComponents}>(
    ({hit, components}) => {
        if (!hit.categories || hit.categories.length <= 1) return null
        return (
            <Breadcrumb
                items={hit.categories.map((_, index) => (
